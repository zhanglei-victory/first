package com.microsec.ycjc.farm.service.impl;

import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;

import com.microsec.ycjc.farm.cond.BreedCond;
import com.microsec.ycjc.farm.dao.IBreedDao;
import com.microsec.ycjc.farm.pojo.Breed;
import com.microsec.ycjc.farm.service.IBreedService;
import com.microsec.ycjc.util.DataGrid;

@Service("breedService")
public class BreedServiceImpl implements IBreedService {

	@Autowired
	private IBreedDao dao;
	
	public IBreedDao getDao() {
		return dao;
	}

	public void setDao(IBreedDao dao) {
		this.dao = dao;
	}

	@Override
	public void insert(Breed info) {
		dao.insert(info);
	}

	@Override
	public void update(Breed info) {
		dao.update(info);
	}

	@Override
	public void delete(String id) {
		
	}

	@Override
	public Breed findById(String id) {
		return dao.findById(id);
	}

	@Override
	public long findAllCountByCond(BreedCond cond) {
		return dao.findAllCountByCond(cond);
	}

	@Override
	public DataGrid findAllInfoByCond(BreedCond cond) {
		List<Breed> list = dao.findAllInfoByCond(cond);
		DataGrid j = new DataGrid();
		j.setRows(list);
		j.setTotal(this.findAllCountByCond(cond));
		return j;
	}

	@Override
	public List<Breed> findAllInfoList() {
		return dao.findAllInfoList();
	}

	@Override
	public List<Breed> findVaccineList(BreedCond cond) {
		return dao.findVaccineList(cond);
	}
}
