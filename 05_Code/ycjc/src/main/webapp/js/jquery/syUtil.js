	$.extend($.fn.validatebox.defaults.rules, { 
		...
		length:{validator:function(value,param){ 
			...
			message:"输入内容长度必须在{0}到{1}之间." 
		}, 
		length2:{validator:function(value,param){ 
			var len=$.trim(value).length; 
				return len>=param[0]&&len<=param[1]; 
			}, 
			message:"输入内容长度必须在{0}到{1}字之间." 
		},
		open:{validator:function(value,param){ 
			var len=$.trim(value); 
				return len>=param[0]&&len<=param[1]; 
			}, 
			message:"开启高度必须在{0}到{1}之间." 
		},
		close:{validator:function(value,param){ 
			var len=$.trim(value); 
				return len>=param[0]&&len<=param[1]; 
			}, 
			message:"关闭高度必须在{0}到{1}之间." 
		},
		phone : {// 验证电话号码 
